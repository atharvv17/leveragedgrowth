...
import { Globe, CreditCard, Phone, Linkedin, Database, GitBranch, BookOpen } from 'lucide-react';
const SolutionsGrid: React.FC = () => {
  const solutions = [{
    ...
    description: ["Automated data scraping for high-quality leads.", "AI chatbots for instant lead qualification.", "CRM integration & automated follow-ups."],
    ctaText: "Book a Demo",
    ctaLink: "https://calendly.com/leveragegrowthbiz/30min",
  ...
  return <section className="px-6 lg:px-24 relative md:px-[16px] my-0 py-px">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {solutions.map((solution, index) => <SolutionCard key={index} title={solution.title} subtitle={solution.subtitle} icon={solution.icon} description={solution.description} ctaText={solution.ctaText} ctaLink={solution.ctaLink} accentColor={solution.accentColor} delay={index * 100} />)}
        </div>
      </div>
    </section>;
};
